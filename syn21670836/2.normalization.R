...
#By patient
plot.size.patient.BN <- ggplot(coldata.norm, aes(x=PatientID, y=nCountAF, fill=group.sex)) +
  geom_boxplot() +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  xlab("Paciente")+
...
plot.size.patient.AN <- ggplot(coldata.norm, aes(x=PatientID, y=nCountnorm, fill=group.sex)) +
  geom_boxplot() +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  xlab("Paciente")+
...
methods.comp <- ggplot(coldata.norm, aes(
  x = library.size.factors, y = deconvolution.size.factors)) +
  geom_point() +
  geom_abline(intercept=0, slope=1, color="black") +
  theme_classic() +
  xlab("Factor de normalización por tamaño de librería")+
  ylab("Factor de normalización por deconvolución")
methods.comp
jpeg(file = './figures/2.normalization/methods.comp.jpeg', width=10, height=6, units="in", res=300)
print(methods.comp)
dev.off()
svg(file = './figures/2.normalization/methods.comp.svg', width=10, height=6)
print(methods.comp)
dev.off()
