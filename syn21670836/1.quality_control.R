...
sce.raw@colData$nCount <- Matrix::colSums(counts(sce.raw))

# Number of genes per cell
sce.raw@colData$nFeatures <- Matrix::colSums(counts(sce.raw)>0)
...
ggsave('./figures/1.quality_control/manuscript_nCounts_BF.jpeg', plot = plot.nCount.groupsex.BF, 
       width = 10, height = 6, units = 'in', device = 'jpeg', dpi = 300)
ggsave('./figures/1.quality_control/manuscript_nCounts_BF.svg', plot = plot.nCount.groupsex.BF, 
       width = 10, height = 6, units = 'in', device = 'svg')
...
ggsave('./figures/1.quality_control/manuscript_mito_BF.jpeg', plot = plot.mito.groupsex.BF, 
       width = 10, height = 6, units = 'in', device = 'jpeg', dpi = 300)
ggsave('./figures/1.quality_control/manuscript_mito_BF.svg', plot = plot.mito.groupsex.BF, 
       width = 10, height = 6, units = 'in', device = 'svg')
...
# Correcr library, nCounts
discard.count <- sce.qc$sum < 400 | sce.qc$sum > 20000
sce.qc$discard.count <- discard.count
print("Células eliminadas por tamaño librería:")
table(discard.count)

# Correct nFeatures
discard.ngenes <- sce.qc$detected < 400 | sce.qc$detected > 7000
sce.qc$discard.ngenes <- discard.ngenes
print("Células eliminadas por número de genes:")
table(discard.ngenes)
...
discard = discard.count | discard.ngenes | discard.mito | discard.doublet
...
# Discard those genes that are not expressed in at least 3 cells
print("Genes a descartar por aparecer en menos de 3 celulas")
selected.features <-  nexprs(sce.raw,byrow = TRUE,detection_limit = 1) >= 3
...
ggsave('./figures/1.quality_control/manuscript_nCounts_AF.jpeg', plot = plot.nCount.groupsex.AF, 
       width = 10, height = 6, units = 'in', device = 'jpeg', dpi = 300)
ggsave('./figures/1.quality_control/manuscript_nCounts_AF.svg', plot = plot.nCount.groupsex.AF, 
       width = 10, height = 6, units = 'in', device = 'svg')
...
ggsave('./figures/1.quality_control/manuscript_mito_AF.jpeg', plot = plot.mito.groupsex.AF, 
       width = 10, height = 6, units = 'in', device = 'jpeg', dpi = 300)
ggsave('./figures/1.quality_control/manuscript_mito_AF.svg', plot = plot.mito.groupsex.AF, 
       width = 10, height = 6, units = 'in', device = 'svg')
...
coldata_plot <- coldata %>%
  mutate(
    status = case_when(
      ...
      discard.ngenes == TRUE ~ "Descartado por número de genes",
    ...
    status = factor(status, levels = c(
      ...
      "Descartado por número de genes",
...
discard.plot.colors <- ggplot(coldata_plot, aes(x = factor(PatientID), fill = status)) +
  ...
  scale_fill_manual(
    values = c(
      ...
      "Descartado por número de genes" = palette5[3],
