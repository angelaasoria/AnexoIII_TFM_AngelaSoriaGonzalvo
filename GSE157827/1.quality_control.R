...
sce.raw$condition <- factor(sce.raw$condition, levels = c("AD", "Control"))
sce.raw$group.sex <- factor(sce.raw$group.sex, levels = c("AD_Hombre", "Control_Hombre",
                                                          "AD_Mujer", "Control_Mujer"))
...
# Discards of total cells
discard = discard.mito | discard.ngenes | discard.count | discard.doublet
...
# Discard the genes that are expressed in less than 2 cells
...
table(selected.features) # FALSE are the discarded ones
...
# Discard the genes.discard and the ones that are FALSE in selected.features
...
# We keep the rows/features in nkeep and the columns/cells that are NOT discard
