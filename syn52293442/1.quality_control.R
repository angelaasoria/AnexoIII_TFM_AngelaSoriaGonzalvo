...
sce.raw <- scDblFinder::scDblFinder(sce.raw, samples = 'projid')
...
# Parameters of mitochondrial, ribosomal and ERCC genes
is.mito <- grepl("^MT-", rownames(sce.raw))
mito.genes <- rownames(sce.raw)[is.mito]

is.ribo <- grepl("^RP[SL]", rownames(sce.raw))
ribo.genes <- rownames(sce.raw)[is.ribo]

is.ercc <- grepl("^ERCC", rownames(sce.raw))
ercc.genes <- rownames(sce.raw)[is.ercc]
...
jpeg('./figures/1.quality_control/nFeatures_density_BF.jpeg', 
...
svg('./figures/1.quality_control/nFeatures_density_BF.svg', 
    width = 10, height = 6)
plot(density(sce.raw$nFeatures), 
     xlab = 'Número de genes', 
     ylab = 'Densidad')
...
plot.nFeatures.patient.BF <- plotColData(
  sce.raw, x = 'projid', y = 'nFeatures', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Número de genes') +
...
jpeg('./figures/1.quality_control/nCounts_density_BF.jpeg', 
     width = 10, height = 6, units = "in", res = 300)
...
svg('./figures/1.quality_control/nCounts_density_BF.svg', 
    width = 10, height = 6)
plot(density(sce.raw$nCount), 
     xlab = 'Tamaño de librería', 
     ylab = 'Densidad')
...
plot.nCount.patient.BF <- plotColData(
  sce.raw, x = 'projid', y = 'nCount', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Tamaño de librería') +
  theme(legend.position = 'right',
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
plot.mito.patient.BF <- plotColData(
  sce.raw, x = 'projid', y = 'subsets_MITO_percent', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Fracción de genes mitocondriales') +
  theme(legend.position = 'right',
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
plot.ncells.patient <- ggplot(coldata, aes(x = projid, fill = group.sex)) +
  geom_bar() +
  scale_fill_manual(values = palette4) +
  labs(
    x = 'Paciente',
...
# Correct by ribosomal reads.
discard.ribo <- sce.qc$subsets_RIBO_percent > 20
sce.qc$discard.ribo <- discard.ribo
print("Células eliminadas por proporción ribosomal:")
table(discard.ribo)
...
# Total cells discarded
discard = discard.mito | discard.ribo | discard.doublet
...
pseudogenes <- rownames(sce.raw)[grep('P$', rownames(sce.raw))]
# Processed pseudogenes
pseudogenes1 <- rownames(sce.raw)[grep("P[0-9]+$",rownames(sce.raw))]
#tRNA
trna <- rownames(sce.raw)[grep('^TR.-', rownames(sce.raw))]
# Small nuclear RNA
snrna <- rownames(sce.raw)[grep('^RNU', rownames(sce.raw))]
# Small nucleolar RNA
snorna <- rownames(sce.raw)[grep("^SNORD",rownames(sce.raw))] # SNORD# for “small nucleolar RNA, C/D box” genes
snorna1 <- rownames(sce.raw)[grep("^SNORA",rownames(sce.raw))] # SNORA# for “small nucleolar RNA, H/ACA box” genes
snorna2 <- rownames(sce.raw)[grep("^SCARNA",rownames(sce.raw))] # SCARNA# for “small Cajal body‐specific RNA” genes
#Ribosomal RNA
rrna <- rownames(sce.raw)[grep('^RNA[0-9]', rownames(sce.raw))]
# (pseudo?)genes not described https://www.biostars.org/p/51456/
rprna <- rownames(sce.raw)[grep("^RP[0-9]",rownames(sce.raw))]
# Mitochondrial genes
mito.genes <- rownames(sce.raw)[grepl("^MT-", rownames(sce.raw))]
# Ribosomal genes
ribo.genes <- rownames(sce.raw)[grepl("^RP[SL]", rownames(sce.raw))]
# ERCC genes
ercc.genes <- rownames(sce.raw)[grepl("^ERCC", rownames(sce.raw))]


# Discard genes that are expressed in less than 2 cells, following the paper
print("Genes a descartar por aparecer en menos de 2 células")
...
not.selected.pseudogenes <- rownames(sce.raw) %in% genes.discard
print('Genes a descartar finales:')
# Discard the ones in genes.discard and the ones that are FALSE in selected.features
...
# Keep the rows/features in nkeep and the columns/cells that are NOT discard
...
plot.ncells.patient.AF <- ggplot(coldata.filt, aes(x = projid, fill = group.sex)) +
  geom_bar() +
  scale_fill_manual(values = palette4) +
  labs(
    x = 'Paciente',
...
plot.nFeatures.patient.AF <- plotColData(
  sce.filt, x = 'projid', y = 'nFeatures', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Número de genes') +
  theme(legend.position = 'right',
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
plot.nCount.patient.AF <- plotColData(
  sce.filt, x = 'projid', y = 'nCount', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Tamaño de librería') +
  theme(legend.position = 'right',
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
plot.mito.patient.AF <- plotColData(
  sce.filt, x = 'projid', y = 'subsets_MITO_percent', colour_by = 'group.sex') +
  xlab('Paciente') +
  ylab('Fracción de genes mitocondriales') +
  theme(legend.position = 'none',
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
coldata <- colData(sce.qc)
coldata.df <- as.data.frame(coldata)
discard.plot <- ggplot(coldata.df, aes(x = factor(projid), fill = discard)) +
...
coldata <- as_tibble(as.data.frame(coldata))
coldata_plot <- coldata %>%
  mutate(
    status = case_when(
      ...
      discard.ribo == TRUE ~ "Descartado por % ribosomal",
      discard.doublet == TRUE ~ "Descartado por doblete"
    ),
    status = factor(status, levels = c(
      "Conservado",
      "Descartado por % mitocondrial",
      "Descartado por % ribosomal",
      "Descartado por doblete"
    ))
  )

discard.plot.colors <- ggplot(coldata_plot, aes(x = factor(projid), fill = status)) +
  ...
  scale_fill_manual(
    values = c(
      ...
      "Descartado por % ribosomal" = palette5[3],
