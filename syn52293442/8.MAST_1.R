...
for (i in 1:length(cell.types)){
  ...
  #------------------------- Get distance of each transcript -------------------
  ...
  valid_genes <- !is.na(rowData$ensembl.id) & !is.na(rowData$gene.symbol)
  sce.dge <- sce.dge[valid_genes, ]
  rowData <- rowData[valid_genes, ]
  
  rownames(sce.dge) <- rowData$ensembl.id
  ...
  ## Remove duplicates
  transcript <- transcript[!duplicated(transcript$Gene.stable.ID) & !is.na(transcript$Gene.stable.ID),]
  ...
  x <- rownames(sce.dge) %in% rownames(transcript) 
  sce.dge  <- sce.dge[x, ]
  
  # Keep common genes in DATABASE
  x <- rownames(transcript) %in% rownames(sce.dge)  
  ...
  zlmCond = zlm(~0+group.sex+cngeneson+projid, sce.assay_cells, exprs_values = "log2tpm", method = "bayesglm", ebayes = TRUE, parallel = TRUE)
  # Save the model
