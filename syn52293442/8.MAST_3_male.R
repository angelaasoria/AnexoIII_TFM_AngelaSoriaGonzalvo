...
cell.types <- c('Oligodendrocitos', 'Inhibidoras', 'Excitadoras')
...
for (i in 1:length(cell.types)){
  ...
  # Test hypothesis
  zlmCond_cells <- lrTest(zlmCond, Hypothesis("group.sexAD_Hombre - group.sexControl_Hombre"))
  saveRDS(zlmCond_cells, file = paste0("./figures/8.MAST/", cell.type,"/zlmCond_cells_M.RDS"))
  results = extract_results(zz = zlmCond_cells)
  
  # Sacamos LogFC con CPM
  LFC.sF = LFC.standard(data = sce.dge, results = results, case = "AD_Hombre", control = "Control_Hombre")
  
  
  # Sacamos LogFC modo facherito (Fer)
  LFC.mF = LFC.MAST(zlm = zlmCond, control = "group.sexControl_Hombre", case = "group.sexAD_Hombre", results = results)
  ...
  write.table(results, file=paste0(path,cell.type,"res_M.tsv"), col.names=TRUE, row.names=FALSE, sep="\t")
  write.table(results[results$p.adjusted < 0.05,], file=paste0(path,cell.type,"res_M_sig.tsv"), col.names=TRUE, row.names=FALSE, sep="\t")
  print("MAST para hombre completado")
