...
# Normalizamos por factor de librería cada célula
...
sce.norm@colData$nCountAF <- colSums(counts(sce.filt))
...
methods.condition <- ggplot(coldata.norm, aes(
  ...
  scale_color_manual(values = palette4, name = "Condición", labels = c('AD', 'Control'))
